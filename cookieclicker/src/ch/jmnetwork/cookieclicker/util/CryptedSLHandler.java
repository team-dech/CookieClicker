    ...
    public void load() {
        ...
        if (data != null) {
            ...
            try {
                ...
                if (next && !stop) cmanager.setTotalCookies(dis.readLong());
                else cmanager.setTotalCookies(0);
                if (!next && !stop) stop = true;

                next = dis.readBoolean();
                if (next && !stop) cmanager.setHandmadeCookies(dis.readLong());
                else cmanager.setHandmadeCookies(0);
                if (!next && !stop) stop = true;

                next = dis.readBoolean();
                if (next && !stop) CookieClickerMain.TICKS_PER_SECOND = dis.readInt();
                else CookieClickerMain.TICKS_PER_SECOND = 25;
                if (!next && !stop) stop = true;

                next = dis.readBoolean();
                if (next && !stop) Helper.owned[0] = dis.readInt();
                else Helper.owned[0] = 0;
                if (!next && !stop) stop = true;

                next = dis.readBoolean();
                if (next && !stop) Helper.owned[1] = dis.readInt();
                else Helper.owned[1] = 0;
                if (!next && !stop) stop = true;

                next = dis.readBoolean();
                if (next && !stop) Helper.owned[2] = dis.readInt();
                else Helper.owned[2] = 0;
                if (!next && !stop) stop = true;

                next = dis.readBoolean();
                if (next && !stop) Helper.owned[3] = dis.readInt();
                else Helper.owned[3] = 0;
                if (!next && !stop) stop = true;

                next = dis.readBoolean();
                if (next && !stop) Helper.owned[4] = dis.readInt();
                else Helper.owned[4] = 0;
                if (!next && !stop) stop = true;

                next = dis.readBoolean();
                if (next && !stop) Helper.owned[5] = dis.readInt();
                else Helper.owned[5] = 0;
                if (!next && !stop) stop = true;

                next = dis.readBoolean();
                if (next && !stop) Helper.owned[6] = dis.readInt();
                else Helper.owned[6] = 0;
                if (!next && !stop) stop = true;

                next = dis.readBoolean();
                if (next && !stop) Helper.owned[7] = dis.readInt();
                else Helper.owned[7] = 0;
                if (!next && !stop) stop = true;

                next = dis.readBoolean();
                if (next && !stop) Helper.owned[8] = dis.readInt();
                else Helper.owned[8] = 0;
                if (!next && !stop) stop = true;

                next = dis.readBoolean();
                if (next && !stop) Helper.owned[9] = dis.readInt();
                else Helper.owned[9] = 0;
                if (!next && !stop) stop = true;
            } catch (Exception e) {
                e.printStackTrace();
            }
